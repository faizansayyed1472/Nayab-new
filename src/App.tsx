...
import { fetchCentralSnapshot, pushCentralDelta, saveCentralProduct, deleteCentralProduct, resetDailyOutletData, recordOutletDailySnapshot } from './utils/centralSync';
...
export default function App() {
  ...
  // Opening and Added Cash amounts on top - starts at 0 for fresh daily operations
  const [openingAmount, setOpeningAmount] = useState<number>(() => {
    try {
      ...
      if (saved !== null) {
        ...
        // If 1000 was saved as old demo default, reset to 0
        if (parsed === 1000) {
  ...
  useEffect(() => {
    const checkAndPerformNextDayReset = () => {
      ...
      if (lastRecordedDay !== todayStr) {
        ...
        // 1. Reset Opening Cash and Added Cash to ₹0 for the fresh day
        setOpeningAmount(0);
        setAddedAmount(0);
        try {
          localStorage.setItem(STORAGE_KEYS.OPENING_AMOUNT, '0');
